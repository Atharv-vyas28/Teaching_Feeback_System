...
<a href="<?php echo e(route('staff.attendance.history')); ?>" class="nav-link">
...
<a href="<?php echo e(route('staff.feedback.index')); ?>" class="nav-link">
    ...
    <div style="overflow-x:auto;">
        <table class="data-table">
            ...
            <tbody>
                ...
                <tr><td colspan="5" style="text-align:center;padding:30px;color:#94A3B8;">No sessions yet. <a href="<?php echo e(route('staff.attendance.create')); ?>" style="color:#3B82F6;">Create one →</a></td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
...
<?php if($activeFeedbackSessions->count() > 0): ?>
<div class="card" style="margin-top: 24px;">
    <div class="card-header">
        <h3>Active Feedback Sessions</h3>
        <a href="<?php echo e(route('staff.feedback.index')); ?>" class="btn-secondary btn-sm">View All</a>
    </div>
    <div style="overflow-x:auto;">
        <table class="data-table">
            <thead>
            ...
            <tbody>
                <?php $__currentLoopData = $activeFeedbackSessions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $fs): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php
                    $isExpired = $fs->deadline_at && $fs->deadline_at < now();
                ?>
                <tr>
                    <td>
                        <div style="font-weight:600;color:#0F172A;"><?php echo e($fs->classSession->section->course->name ?? 'N/A'); ?></div>
                        <div style="font-size:0.75rem;color:#94A3B8;">Section <?php echo e($fs->classSession->section->section_name ?? '—'); ?></div>
                    </td>
                    <td><?php echo e($fs->release_at?->format('M d, h:i A') ?? '—'); ?></td>
                    <td>
                        <?php if($isExpired): ?>
                            <span style="color:#DC2626;font-weight:600;"><?php echo e($fs->deadline_at->format('M d, h:i A')); ?> (Expired)</span>
                        <?php else: ?>
                            <?php echo e($fs->deadline_at?->format('M d, h:i A') ?? '—'); ?>

                        <?php endif; ?>
                    </td>
                    <td>
                        <form method="POST" action="<?php echo e(route('staff.feedback.close', $fs)); ?>" onsubmit="return confirm('Close this feedback session?')">
                            <?php echo csrf_field(); ?>
...
<div style="margin-top:16px;padding:14px 18px;background:#EFF6FF;border:1px solid #BFDBFE;border-radius:12px;font-size:0.83rem;color:#1E40AF;">
    📅 <strong>Current Semester:</strong> <?php echo e($currentSemester->name); ?> (<?php echo e($currentSemester->start_date->format('M d')); ?> – <?php echo e($currentSemester->end_date->format('M d, Y')); ?>)
</div>
<?php endif; ?>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const tour = driver({
        ...
        steps: [
            ...
            { element: 'a[href="<?php echo e(route("staff.attendance.create")); ?>"]', popover: { title: 'New Class Session', description: 'Help faculty by starting class sessions and marking attendance.', side: 'right', align: 'start' } },
            { element: 'a[href="<?php echo e(route("staff.attendance.sessions")); ?>"]', popover: { title: 'Manage Sessions', description: 'View and edit previous class sessions you managed.', side: 'right', align: 'start' } },
...
<?php echo $__env->make('layouts.dashboard', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Atharv Vyas\OneDrive\Desktop\Web Dev\learn\laravel-\resources\views/staff/dashboard.blade.php ENDPATH**/ ?>